...
bool VpnClient::IsStarted() {
  ...
  const std::unique_lock<std::mutex> lock(state_mutex_);  // mutex
...
bool VpnClient::Start() {
...

  {
    ...
    if (running_ || !http_client_ || !virtual_net_interface_) {
      return false;
    }
...
bool VpnClient::Stop() {
...

  {
    ...
    virtual_net_interface = std::move(virtual_net_interface_);
    http_client = std::move(http_client_);
  }
  ...
  if (virtual_net_interface) {
    SPDLOG_INFO("Stopping virtual network interface");
    virtual_net_interface->Stop();
...
void VpnClient::HandlePacketFromWebSocket(
  ...
  std::unique_lock<std::mutex> lock(ws_queue_mutex_);  // mutex
